...
process_query()
{
    ...
    if [ "$DEBUG" = true ]; then
        echo "curl -sSLN -w "%{http_code}"  \"${ENDPOINT}\" --tls-max 1.2 --tlsv1.2  --ciphers ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-RSA-AES128-GCM-SHA256:ECDHE-ECDSA-CHACHA20-POLY1305:ECDHE-RSA-CHACHA20-POLY1305 -H \"Content-type: application/json\" -H \"cache-control: no-cache\" -X POST --data \"${query}\""
    fi
    
    data=$(curl -sSLN -w "%{http_code}" "${ENDPOINT}" --tls-max 1.2 --tlsv1.2  --ciphers ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-RSA-AES128-GCM-SHA256:ECDHE-ECDSA-CHACHA20-POLY1305:ECDHE-RSA-CHACHA20-POLY1305 -H "Content-type: application/json" -H "cache-control: no-cache" -X POST --data "${query}")
